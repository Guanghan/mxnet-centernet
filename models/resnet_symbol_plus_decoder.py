...
import sys
sys.path.insert(0, "/export/guanghan/CenterNet-Gluon/models")
sys.path.insert(0, "/Users/guanghan.ning/Desktop/dev/CenterNet-Gluon/models")
from decoder import symbolic_decode_centernet_pose
...
class BottleneckV1(HybridBlock):
    def __init__(self, channels, stride, downsample=False, in_channels=0,
        ...
        self.body = nn.HybridSequential(prefix='')
        self.body.add(nn.Conv2D(channels//4, kernel_size=1, strides=stride, in_channels=in_channels))
        self.body.add(norm_layer(**({} if norm_kwargs is None else norm_kwargs)))
        self.body.add(nn.Activation('relu'))
        self.body.add(_conv3x3(channels//4, 1, channels//4))
        self.body.add(norm_layer(**({} if norm_kwargs is None else norm_kwargs)))
        self.body.add(nn.Activation('relu'))
        self.body.add(nn.Conv2D(channels, kernel_size=1, strides=1, in_channels=channels // 4))
...
def get_pose_net(num_layers, heads, head_conv=256, load_pretrained = True, ctx = cpu()):
    ...
    if num_layers == 18 and load_pretrained:
        print("Loading pretrained model: resnet 18")
