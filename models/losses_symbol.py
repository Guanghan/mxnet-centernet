...
class MultiPoseLoss(nn.Block):
    ...
    def forward(self, outputs,
        ...
        for s in range(opt.num_stacks):
            ...
            hm_loss = hm_loss + self.crit(output[0], targets_center) / opt.num_stacks
            hp_loss = hp_loss + self.crit_kp(output[2], targets_poserel_mask, targets_inds, targets_poserel) / opt.num_stacks

            if opt.wh_weight > 0:
                wh_loss = wh_loss + self.crit_reg(output[1], targets_2d_wh_mask, targets_inds, targets_2d_wh) / opt.num_stacks

            if opt.reg_offset and opt.off_weight > 0:
                off_loss = off_loss + self.crit_reg(output[3], targets_2d_wh_mask, targets_inds, targets_2d_offset) / opt.num_stacks

            if opt.reg_hp_offset and opt.off_weight > 0:
                hp_offset_loss = hp_offset_loss + self.crit_reg(output[5], targets_posemap_mask, targets_posemap_ind, targets_posemap_offset) / opt.num_stacks

            if opt.hm_hp and opt.hm_hp_weight > 0:
                hm_hp_loss = hm_hp_loss + self.crit_hm_hp(output[4], targets_posemap) / opt.num_stacks
